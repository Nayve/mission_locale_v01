	...
	*/	
	...
	function gdlr_get_session_info($array = array(), $option=array(), $speakers=array(), $wrapper = true){ 
		...
		foreach($array as $post_info){	
			...
			switch( $post_info ){
				case 'date':
					if(empty($option['session-date'])) break;
					$session_date = strtotime($option['session-date'] . ' 00:00:00');
					$session_end_date = strtotime($option['session-end-date'] . ' 00:00:00');
					...
					$ret .= " au ";						
					$ret .= "<strong>";						
					$ret .= date_i18n($theme_option['date-format'], $session_end_date);						
					$ret .= "</strong>";						
					$ret .= '</div>';	
				...
				case 'location':
					...
					$ret .= $option['location'];						
					$ret .= "<br>";						
					$ret .= $option['location2'];						
					$ret .= '</div>';		

					if(empty($option['session-info'])) break;
					$ret .= '<div class="session-info session-location">';
					$ret .= "<strong>";	
					$ret .= 'Séance d\'information :';
					$ret .= "</strong>";
					$ret .= "<br>";		
					$ret .= $option['session-info'];											
					$ret .= '</div>';							
